...
const Drivers = {
  SHT45: {
    ADDRESSES: {
      value: [0x44] // The default I2C address for SHT45
    },
    REGISTER: {
      value: {
        // Add registers specific to the SHT45
        SOFT_RESET: 0x94, // Soft reset command
        MEASURE_HIGH_REPEATABILITY: 0xFD, // High precision measurement command
        // You can add more commands and registers as needed
      }
    },
    initialize: {
      value(board, options) {
        ...
        // Soft reset to ensure the sensor is in a known state
        io.i2cWrite(address, this.REGISTER.SOFT_RESET);

        const readCycle = () => {
          io.i2cWrite(address, this.REGISTER.MEASURE_HIGH_REPEATABILITY);

          setTimeout(() => {
            io.i2cReadOnce(address, READLENGTH, data => {
              // Store raw data for temperature and humidity
              const temperatureRaw = uint16(data[0], data[1]);
              const humidityRaw = uint16(data[3], data[4]);

              this.emit("data", { temperature: temperatureRaw, humidity: humidityRaw });
              readCycle();
            });
          }, 16); // Adjust the delay as per sensor specs
        };
    ...
    identifier: {
      value(options) {
        const address = Drivers.addressResolver(Drivers.SHT45, options);
        return `sht-45-${address}`;
      }
    }
  },
...
const Controllers = {
  ...
  SHT45: {
    initialize: {
      value(options) {
        Components.call(this, "SHT45", options);
      }
    },
    components: {
